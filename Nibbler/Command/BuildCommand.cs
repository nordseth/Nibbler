...
{
    private const string DefaultIgnoreFile = ".dockerignore";
    ...
    public CommandOption ToImages { get; private set; }
    ...
    public CommandOption IgnoreFile { get; private set; }

    public CommandOption Label { get; private set; }
    public CommandOption Labels { get; private set; }
    ...
    public ValidationResult Validate(ValidationContext context)
    {
        ...
        if (!ToImage.HasValue() && !ToFile.HasValue() && !ToImages.HasValue())
        {
            validationErrors.Add(($"--{ToImage.LongName}, --{ToFile.LongName} or --{ToImages.LongName} is required.", new[] { ToImage.LongName, ToFile.LongName, ToImage.LongName }));
        }
    ...
    public async Task<int> ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            run.IgnoreFile = IgnoreFile.HasValue() ? IgnoreFile.Value() ?? DefaultIgnoreFile : null;
    ...
    private void SetImageDest(BuildRun run)
    {
        ...
        else if (ToFile.HasValue())
        {
            run.AddFileImageDest(ToFile.Value());
        }
        else
        {
